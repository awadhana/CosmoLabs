 ...
 * full submission and (b) a customer confirmation. Both sends are best-effort:
 * this module NEVER throws and returns {sent:false, reason} when email is
 * unavailable or fails — an email outage must not fail the intake.
 */
...
export async function sendIntakeEmails({ submission, notifyEmail }) {
  try {
    ...
    if (!user || !pass) return { sent: false, reason: "GMAIL_USER / GMAIL_APP_PASSWORD not configured" };
